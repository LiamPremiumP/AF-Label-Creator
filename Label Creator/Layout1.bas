...
LayoutChange:
    ...
    'Data to be used for the layout are are called onces in the beginning of the inner loop, to spped up program
    'Data shouldn't be called more than once from Sata sheet
    
    Sheets("Layout" & Layout).Shapes("TextBox " & 27).TextFrame.Characters.Text = Sheets("Control").Cells(16, 6).Value 'Date
    Dim ControlToDatadata(40)
    ControlToDatadata(9 - 9) = Sheets("Data").Cells(Row, 1).Value 'Bus name
    Dim IdLength As Integer
    IdLength = Len(ControlToDatadata(9 - 9))

    ControlToDatadata(11 - 9) = Round((Sheets("Data").Cells(Row, 3).Value), 1) 'Voltage
    
    Dim voltage As Integer
    voltage = ControlToDatadata(11 - 9) * 1000

    ControlToDatadata(25 - 9) = Round((Sheets("Data").Cells(Row, 17).Value), 0) 'Arc Flash Boundary
    ControlToDatadata(26 - 9) = "Worst Case Arc Incident Energy at Nominal Working Distance (" & Round(Sheets("Data").Cells(Row, 18).Value, 0) & " cm from source)" 'Working Distance
    
    Dim ppe As Double
    ppe = Sheets("Data").Cells(Row, 19).Value   'Round((Sheets("Data").Cells(Row, 19).Value), 1)
    ControlToDatadata(27 - 9) = ppe
    If ppe < 10 Then
        ControlToDatadata(27 - 9) = Format(ppe, "0.00") ' ppe (cal/cm)
    ElseIf ppe >= 10 Then
        ControlToDatadata(27 - 9) = Format(ppe, "0.0")
    End If

    ControlToDatadata(28 - 9) = Sheets("Data").Cells(Row, 20).Value 'PPE Level (Category)
    ...
    Dim DCAC As String
    DCAC = Sheets("Data").Cells(Row, 26).Value 'Dc or Ac


    ControlToDatadata(36 - 9) = vbCrLf & Sheets("Data").Cells(Row, 27).Value 'Document Number

    ControlToDatadata(37 - 9) = Sheets("Data").Cells(Row, 28).Value  'Standard
    Dim standard As Integer
    If ControlToDatadata(37 - 9) = "NFPA" Then
        standard = 26
    ElseIf ControlToDatadata(37 - 9) = "ISEN" Then
        standard = 47
    Else
        MsgBox "Re-enter standard"
    End If


    Dim placeh As Integer
    For placeh = 0 To 16
                    If voltage >= Sheets("Register").Cells(placeh + standard, 1).Value And voltage < Sheets("Register").Cells(placeh + standard + 1, 1).Value Then
                    
                        
                        ControlToDatadata(33 - 9) = Sheets("Register").Cells(placeh + standard, 3).Value * 100 'limited approach
                        
                        ControlToDatadata(34 - 9) = Sheets("Register").Cells(placeh + standard, 4).Value  'restricted apporach
                        If IsNumeric(ControlToDatadata(34 - 9)) = True Then
                        ControlToDatadata(34 - 9) = CStr(ControlToDatadata(34 - 9)) + " cm"
                        End If
                    End If
    Next placeh
...
titleskip:
        ...
        'Adds or removes maintenance PPE IF the same
            ...
            If ControlToDatadata(k - 9) = False Then
                MsgBox "You forgot to enter data into " & Row & ". If you didn't, turn it off at row " & k & " in layout " & Layout & " in control sheet"
                GoTo Exit1
            End If
            ...
            'Adds the DC/AC
            If k = 11 Then
                Sheets("Layout" & Layout).Shapes("TextBox " & box).TextFrame.Characters.Text = Sheets("Layout" & Layout).Shapes("TextBox " & box).TextFrame.Characters.Text + DCAC
                End If
        
            If k = 37 Then
                Sheets("Layout" & Layout).Shapes("TextBox " & 34).TextFrame.Characters.Text = (Sheets("Control").Cells(k, sCol + 3).Value) & Sheets("Register").Cells(standard - 1, 1).Value & (Sheets("Control").Cells(k, sCol + 2).Value)
                Sheets("Layout" & Layout).Shapes("TextBox " & 36).TextFrame.Characters.Text = Sheets("Register").Cells(standard - 1, 3).Value
                Sheets("Layout" & Layout).Shapes("TextBox " & 40).TextFrame.Characters.Text = Sheets("Register").Cells(standard - 1, 4).Value
...
nosafe:
    ...
    'Save As PDF Document
        ActivePresentation.ExportAsFixedFormat GetFolder & "\" & Sheets("Control").Range("C24").Value & "-" & Sheets("Data").Cells(Row, 1).Value & "-" & Sheets("Data").Cells(Row, 27).Value & ".pdf", _
